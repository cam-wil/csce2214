...
loop:
    ...
    lw      $t3,    0($s0)           #current value of var1[i]
    lw      $t4,    0($s1)           #current value of var2[i]

    #do the math and store it in result[i]
    sub     $t5,    $t3, $t4         #subracts value in var1 from value in var2
    sw      $t5,     0($s2)          #stores value of t5 to current location in result array

    #do the loop stuff
    addi $t2, $t2, 4            #increments byte offset counter by one word
